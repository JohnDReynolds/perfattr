...
allocation, selection, interaction, and total in `expected_effects.csv`.
...
allocation `0.30 * (0.06 - 0.036) = 0.0072`, selection
`0.40 * (0.10 - 0.06) = 0.016`, and interaction
`0.30 * (0.10 - 0.06) = 0.012`. In `three_effect_authoritative`, the contributions
imply effective portfolio returns of `0.05` and `0.10` and benchmark returns of `0.04`
and `0.06`; the supplied input returns intentionally differ and do not determine the
effects.
