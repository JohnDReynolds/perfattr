"""Tests for the public single-period attribution vertical slice."""
...
def test_result_frames_follow_the_specified_single_period_contract() -> None:
...
def test_calculation_is_deterministic_and_does_not_mutate_inputs() -> None:
    ...
    pd.testing.assert_frame_equal(ordered.period_detail, shuffled.period_detail)
...
def test_multi_period_input_reports_the_vertical_slice_limit() -> None:
    """The first slice should reject, rather than partially calculate, many periods."""
    portfolio, benchmark = _read_inputs("multi_period_linking")

    with pytest.raises(AttributionError, match="supports exactly one reporting period"):
